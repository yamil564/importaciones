...
class StockLandedCost(models.Model):
    ...
    def _compute_total_amount_prorrateo(self):
        self.amount_subtotal_prorrateo = sum(line.final_cost for line in self.stock_valuario_orderlines)
        self.amount_total_prorrateo = self.amount_subtotal_prorrateo*1.18
    ...
    def compute_landed_cost(self):
        ...
        for cost in self.filtered(lambda cost: cost.picking_ids):
            ...
            for pick in cost.mapped('picking_ids'):
                for move in pick.mapped('move_lines'):
                    former_cost2 += sum(quant.cost * quant.qty for quant in move.quant_ids)
...
class stockValuationOrderline(models.Model):
    ...
    def _compute_name(self):
        self.name = str(self.id) + (self.product_id.code or self.product_id.name or '')
    ...
    def _compute_additional_landed_cost(self):
        self.additional_landed_cost = round( (self.sum_former_cost+self.suma_costes)/(self.sum_former_cost or 1.0) ,4)

    @api.one
    @api.depends('former_cost', 'additional_landed_cost')
    def _compute_final_cost(self):
        self.final_precio = self.former_cost_per_unit * self.additional_landed_cost
        self.final_cost = self.former_cost_per_unit * self.additional_landed_cost * self.quantity
