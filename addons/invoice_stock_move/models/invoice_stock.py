    ...
    def action_stock_receive(self):
        for order in self:
            if not order.invoice_line_ids:
                raise UserError(_('No puede crear una orden vacia. Por favor agregue productos.'))
            if not self.number:
                raise UserError(_('Debe validar la factura.'))
            if not self.invoice_picking_id:
                pick = {
                    'picking_type_id': self.picking_type_id.id,
                    'partner_id': self.partner_id.id,
                    'origin': self.number,
                    'relacion_factura': self.id,
                    'referencia_prov_lots': self.numero_importacion,
                    # 'referencia_prov_lots': self.reference,
                    # 'transportista': self.transportista_id.id,
                    # 'guiaFisica': self.guia,
                    # 'fecha_inicio': self.date_traslado,
...
class facturapicking(models.Model):
    _inherit = 'stock.picking'
    relacion_factura = fields.Many2one('account.invoice', string="Factura")